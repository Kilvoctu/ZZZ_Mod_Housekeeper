"""Inline test data: the frozen changelog snapshot and shared test helpers.

The suite needs no on-disk source; the text round-trips byte-exactly
against Path.read_text(encoding="utf-8") of the original.
"""
...
--------------------
作者: 踩蘑菇网绿林小子制作，Github-GreenLzz-Make
声明: 请勿私自转载，否则后期不再上传。
网址1: https://github.com/hefengchang
网址2: https://www.caimogu.cc/user/1761398.html
--------------------"""


def make_repo(root: Path, changelog_text: str = SNAPSHOT_CHANGELOG, subdir: str | None = None) -> Path:
    """Minimal data-repo dir under root: empty character table + a changelog."""
    base = root / subdir if subdir is not None else root
